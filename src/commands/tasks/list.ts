...
export default class TasksList extends Command {
  static description = 'List and manage your recent assigned tasks'
static examples = [
    '<%= config.bin %> <%= command.id %>',
    '<%= config.bin %> <%= command.id %> --limit 20',
  ]
static flags = {
    all: Flags.boolean({char: 'a', description: 'Show all tasks (including hidden statuses)'}),
    limit: Flags.integer({char: 'n', default: 10, description: 'Number of tasks to show'}),
    space: Flags.boolean({char: 's', description: 'Filter by space (interactive)'}),
  }
private api!: ClickUpApi
  ...
  async run(): Promise<void> {
    ...
    if (flags.space) {
      ...
      const selectedSpace = await p.select({
        message: 'Select a space:',
        options: spaces.map((s) => ({label: s.name, value: s.id})),
      })

      if (p.isCancel(selectedSpace)) {
        p.outro('Cancelled')
        return
      }

      spaceIds = [selectedSpace]
    }

    const spinner = p.spinner()
    spinner.start('Fetching your recent tasks...')
    ...
    // Filter hidden statuses unless --all flag is used
    if (!flags.all) {
    ...
    spinner.stop(`Found ${tasks.length} tasks (showing ${displayTasks.length})`)

    if (displayTasks.length === 0) {
      p.log.info('No tasks assigned to you.')
      p.outro('Done')
      return
    }
